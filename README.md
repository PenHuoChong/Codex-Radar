...
> [!WARNING]
> **侧边对话消耗不会被计入。** 当前版本不覆盖 Codex 右侧临时对话面板的 Token 与 API 等价美元消耗；这类对话若未写入标准会话用量日志，本项目无法准确统计或补算。此限制不指左侧任务列表中的普通持久化会话。
>
> 使用侧边临时对话时，账号额度百分比可能继续增加，但本项目统计金额不会包含对应消耗，因此反推的美元额度也可能偏低或失真。本项目仅被动读取本地日志，不通过反代、请求转发或实时订阅补采这部分用量。
...
- 支持“开始计算—结束计算”，统计指定时间段内全部项目中可从本地会话日志读取的增量（不含右侧临时对话）；开始和结束时分别冻结日志偏移与额度快照，结束后新增日志不改变结果。
...
缓存读取字段依次检查 `cached_input_tokens`、`cache_read_tokens`、`cached_tokens`，使用第一个有效的非负整数（含数字字符串）。明确的 `0` 不回退；缺失、空值和无效值继续查找下一别名。完整 JSON、快速解析和 SQLite 导入采用相同规则，避免将缓存输入误当作未缓存输入计价。
...
- 文件游标另存末尾模型上下文及其时间，即使一批新增字节只有 `turn_context`、下一批才出现 Token，也不会退回旧模型。父任务/任务根的模型继承受调用时间约束，不使用未来模型。此次修复不会自动重写已有非空模型记录；如需纠正旧索引，可在备份需要保留的本地汇总后，主动使用现有“完整重建”（会重建本项目缓存，不删除原始日志）。
...
主界面“项目 / 对话用量…”打开独立窗口。窗口支持按项目名称或对话名称手动输入关键词检索；点击项目查看项目汇总，展开后点击对话查看该对话及其子代理的消耗，沿用累计状态和父子任务去重、逐模型及长上下文计价。右侧同时显示总 Token、API 等价美元和各模型明细。

- 范围支持最近24小时、7天、30天、自定义本地起止时间（`yyyy-MM-dd HH:mm:ss`）和全部时间；最近24小时不是自然日。
- 只有打开窗口、刷新列表、选择项目/对话或点击查询才触发读取；不加入启动任务或5分钟刷新，不修改主界面测量范围。
- “补齐 / 更新全部历史日志”是单独的手动操作，先说明读取范围并确认，再扫描本机仍保留的日志，写入本项目独立查询缓存；不删除原始日志，也不重建主测量索引。首次历史补齐可能较慢，可取消。
- “全部时间”不代表可恢复已经删除的原始日志。请查看查询覆盖说明，未补齐、已清理或无法读取的数据不能当作零消耗。
- 打开查询窗口时会询问是否授权读取标题元数据；批准后仅从本机 `session_index.jsonl` 读取会话 ID 和 `thread_name` 标题到窗口内存，刷新时重新读取并更新名称。拒绝授权或没有标题时回退为“未命名”，会话 ID 只用于内部定位或 tooltip；不读取正文生成标题，名称不写入索引或查询缓存。同名对话仍按各自独立 ID 统计。
- 独立历史查询使用日志明确记录的模式；缺失模式按普通价参考，不继承主测量的临时 Fast 人工选择。API 等价美元仍不等于订阅账单。

关闭独立窗口会释放列表和标题；有后台任务时先取消并清理工作线程。没有打开窗口时不创建查询任务。

### 5 小时与周额度

#### 周美元即时显示：1%参考值与严格校准

点击“查看结果”后，只要本次测量已有正的、有限的 API 等价美元成本，就可以显示周总额参考值：

- 本次有效周增量不足 **1 个百分点**（含0、0.1%、0.99%），使用 **本次测量 API 等价美元 × 100**，并标明“按1%折算”。例如花费 $12.50，参考总额为 $1,250；不会把0.1%当真实分母反推为 $12,500。
- 本次周增量达到1个百分点且严格校准有效，切换为下文的两阶段、快照同边界算法。
- 周窗口缺失、冲突或严格校准暂不可用时，仍可显示上述成本参考值；它不证明套餐实际容量，不伪造百分比、重置时间或已用/剩余美元。价格仅部分可知时保留“部分”标记。
- 参考值随成功的结果刷新更新，独立于严格校准证据存储。新测量、账号或套餐变更清除旧参考值；迟到的其他账号结果不能覆盖当前显示。没有可计算的正美元成本时不捏造数字。

此规则是用户指定的显示假设，不是官方计费规则，也不是额度必然达到该金额的保证。
...
| 套餐/计费方式 | 官方对 5 小时额度的说明 |
|---|---|
| Plus | 提供每 5 小时用量估计 |
| Pro 5x / Pro 20x | 分别提供每 5 小时用量估计，不是固定美元池 |
| Standard Business | 提供每 5 小时用量估计；Business $100 档使用 Pro 5x 估计 |
| Enterprise / Edu（非灵活计费） | 大多数功能的每席位限制与 Plus 相同 |
| Enterprise / Edu（灵活计费） | 无固定速率额度，按 credits 扩展；不能硬套订阅 5 小时美元池 |
| Free / Go | 官方确认包含 Codex，但当前价格页未给出对应的每 5 小时用量表；不能据此断言没有窗口 |
| API Key | 按 API 用量付费，不套用 ChatGPT 订阅 5 小时额度 |

“当前套餐”自动模式会显示已接受的常规额度快照中的具体计划类型，并保留日志原始标识。只写 `pro` 时不猜测 5x/20x；仅有内部标识或来源冲突时如实显示，不按用量大小猜档位。此显示不是认证结果，不额外读取认证文件或发起额度查询。

手动套餐列表始终提供 Free、Go、Plus、Pro、Business/Team、Edu、Enterprise 七类，不再要求日志先出现该套餐。Business 的旧日志标识为 `team`；日志实际出现 `business` 或其他标识时，也保留对应选项。手动选择只过滤已有证据，不能生成缺失的窗口。未找到所选套餐窗口时显示“暂无”，不再误称“来源冲突”。

如果你的账号确实没有 5 小时限制，在“当前套餐”对话框勾选 **“本账号无 5 小时限制”**。5 小时卡显示“不适用（人工确认）”，周额度与全部 Token/API 成本不受影响；不会据此认定所有 Pro 都无此限制。该设置仅本次程序运行有效，账号标签变化时清除；取消勾选即可恢复日志窗口显示。自动识别无法确定时可直接选择套餐，不读取认证文件来强行识别。

同一套餐下重复确认、勾选或取消“无 5 小时限制”，只更新显示，不清空周额度和校准证据，也不触发后台重算。真正更换套餐才会清除不兼容估算并重新查询；主动点击“查看结果”仍按新证据更新。

**套餐规则不等于本地一定有窗口数据。** 5 小时卡仍只使用日志实际提供的、有效且未过期的 `codex` 池窗口；未提供时显示暂无，不按套餐制造百分比或重置时间。周美元参考值与实际窗口证据分开显示。企业计费细分、Pro 倍率等若没有可靠日志证据，仍需用户在官方用量页面确认。

#### 日志没有 Fast/普通模式时

推理强度（包括 Ultra）不等于 Fast。每次开始测量时，所有已收录模型默认选择“确认普通模式”，按用户设定自动通过确认，无需逐项操作；这属于用户指定的计价假设，不是日志观测到的模式。需要 Fast 时再分别修改。明确的日志模式始终优先。

开始测量后，可点击“本次计价模式…”按模型分别选择“未确认”“确认普通模式”或“确认 Fast 模式”，再点击“确认并重新计算”。例如，只确认 Astra 为 Fast，不会自动把 Luna 子代理也改为 Fast。明确的日志模式始终优先；人工确认只补足缺失证据，并在结果与额度中标明“人工确认”。

人工选择只保存在本次程序运行的测量状态中，不写入日志、索引或周期历史；它覆盖本次测量及额度校准快照区间的对应模型，开始新测量后恢复全部普通模式。改变选择会清除旧计价假设下的额度结果并后台重算；结束后的重算仍使用原先冻结的结束边界。主动选择“未确认”时仍可显示带标注的普通价参考额度；周窗口或严格校准缺失时使用明确标记的1%参考策略，不将它冒充日志证明的额度。

并发请求可能晚到并携带较低的额度百分比。程序不会把较低百分比与更晚的调用成本直接配对，也不会给旧百分比换上新时间戳；真正切换计划或进入新重置周期仍按新窗口处理。同一有效窗口的后续观察不会仅因时间戳改变而清空已有校准，已用/剩余美元按当前百分比重新计算。
...
严格额度校准在冻结偏移内采用两阶段固定锚点。例如从26%开始，未到27%且已有成本时显示“本次成本×100”的1%参考值，取代此前优先显示历史25%→26%的方式；到27%使用26%→27%首次校准，随后固定27%快照为累计起点：到28%使用27%→28%的成本除以1%，到30%使用27%→30%的累计成本除以3%。到达锚点但尚无后续增量时保持首次校准结果。锚点之后使用日志实际百分比增量（包括0.1%或0.01%），不再滑动取最近1%，也不补造缺失快照。同百分比重复快照不延长严格成本终点。仅聚合所选两端快照之间的严格成本，主测量Token和美元仍覆盖完整测量范围。没有可靠历史对、百分比下降或价格不完整时不生成新的严格估算；可独立展示明确标记的1%参考值。新周期独立建立锚点，不混入旧周期成本。

若日志直接从26%跳到28%，首次校准使用真实26%→28%的成本和2%增量，并将28%作为后续固定锚点，不补造27%快照。

额度卡按“当前用量 → API等价美元总额度 → 比例外推已用/剩余美元 → 起点与校准增量 → 历史回查状态 → 来源与去重完整性”显示，并标明校准截至时间。“比例外推已用”由总额乘当前百分比得到，不是主测量实际累计成本。重复查询若校准边界、百分比和成本均未改变，显示“校准证据未变化，沿用上次结果”；主金额增加不意味着可以把新增成本除以旧百分比增量。来源`snapshot_delta_usd_estimate`表示美元结果来自快照区间成本和实际百分比增量；`limit_tokens`等Token容量字段不参与美元总额公式。父子代理复制事件先按Token指纹及真实血缘去重，即使时间、模型或turn ID不同也只保留最浅层祖先；没有request/response ID时仍明确标记“请求级去重不完整”。

若同一窗口和重置周期存在不同计划的候选（例如 `pro` 与 `prolite`），仅凭最新时间不能确认当前账号来源。程序展示来源冲突及候选百分比，不将其中一条或其美元估算冒充当前账号额度。不同计划不能通过名称硬编码优先级；计划字段也不证明账号身份。

常规5小时及周额度卡只使用 `limit_id=codex` 的快照与同池校准成本，然后再应用“当前套餐”选择。`codex_bengalfox` 等其他池即使更新时间更晚，也不能覆盖常规额度。常规池没有5小时窗口时显示暂无，不用其他池的0%补位；其他池的原始用量记录仍保留，主测量Token和API等价成本不因该筛选而减少。仅在所有候选都没有额度池标识时兼容旧格式，不把空标识当作任意池的通配符。

刷新时缺失、空值、无效或越界的百分比不会被当作0%。同周期回退快照及其套餐选择来源一起被拒绝，旧估算按已接受窗口保留；新估算仍使用它自己的冻结端点，不把成本配到另一个快照。真正重置后的有效0%可以正常显示，同时清除旧周期估算。本修复不自动修改或回填旧索引中已保存的历史0%记录。

**升级套餐后**，点击“当前套餐：自动识别…”并选择当前实际套餐的日志标识（例如 `pro`），确认后立即更新额度。旧进程继续写入 `prolite` 等旧计划时，不再覆盖已确认计划的额度；主测量Token和美元仍统计全部原有范围。选择仅在本次运行中保留，账号标签变化后清除；选回“自动识别”恢复冲突检查。没有所选计划的当前窗口或完整校准证据时显示不可估，不伪造美元也不使用其他套餐顶替。该选择是人工确认的计划范围，并非读取或验证认证信息。

主测量与额度校准周期相互独立：计划、额度池或重置周期改变后，主测量继续累计，额度只使用新周期最近完成的步长；旧周期估算不会跨周期沿用。计划类型不是账号身份。已知账号标签变化会使旧结果失效，迟到的旧账号查询结果不得覆盖新状态。

额度专用成本在父子规范事件去重后检查周期归属。缺少当前行额度元数据时，仅允许采用该会话此前有效的周期信息，不能从其他会话或父任务猜测。存在归属不明调用时不以部分成本生成完整额度。每个窗口独立显示“本次更新”“沿用上次结果”或具体不可估原因；后台失败保留同周期有效值，不用零美元伪装未知结果。
