...
点击“查看结果”后，周总额按有效证据显示；独立1%参考还必须有正的、有限的套餐折算成本及已验证的不足1%增量（未启用新口径的旧调用仍使用 API 成本）：

- 本次有效周增量不足 **1 个百分点**（含0、0.1%、0.99%），使用 **本次测量套餐折算美元参考 × 100**，并标明“按1%折算，非账单”。例如折算消耗 $12.50，参考总额为 $1,250；不会把0.1%当真实分母反推为 $12,500。旧 API 口径调用仍使用 API 等价美元，不与新口径混算。
- 本次周增量达到1个百分点后，停用固定1%参考，切换为下文的两阶段、快照同边界算法。严格校准暂不可用时保留同周期有效估算并说明原因；没有有效旧值则显示等待同边界校准，不能继续将累计成本乘100，也不能把不匹配的主测量成本直接除以当前增量。
- 起止快照缺失、冲突或增量未知时，不再自动套用1%。只有已验证增量为0至不足1个百分点时才显示上述参考；它不证明套餐实际容量，不伪造百分比、重置时间或已用/剩余美元。价格仅部分可知时保留“部分”标记。
...
人工选择只保存在本次程序运行的测量状态中，不写入日志、索引或周期历史；它覆盖本次测量及额度校准快照区间的对应模型，开始新测量后恢复全部普通模式。改变选择会清除旧计价假设下的额度结果并后台重算；结束后的重算仍使用原先冻结的结束边界。主动选择“未确认”时仍可显示带标注的普通价参考额度；已验证增量在0至不足1个百分点之间时可使用明确标记的1%参考策略。增量未知或已达到1个百分点后不再退回1%参考，不将它冒充日志证明的额度。
...
周额度严格校准在冻结偏移内采用固定锚点及用户指定的滞后终点。以本次从36%开始为例：
...
首次完整步长后固定37%为累计起点，不滑动成最近1%。后续终点上限为 `min(当前百分比, max(锚点+1, 当前百分比-1))`，从上限以内选择实际存在的最大百分比的首次快照。例如当前39.5%、存在38.5%快照时，用37%→38.5%的成本除以1.5%；缺失38.5%时只使用上限以内已有端点，不插值补造快照。尚无可用后续端点时保留首次完整步长结果。5小时保留原有不滞后规则，例如从26%开始，到27%用26%→27%，到30%用27%→30%除以3%。

同百分比重复快照不延长严格成本终点。分子和分母始终对应同一冻结快照区间，主测量Token和美元仍覆盖完整测量范围；因此当前周百分比、本次用量区间及实际校准区间可以不同。刷新时持续展示同周期有效美元，直到新证据覆盖；缺少可靠证据时不捏造校准金额。已验证本次增量为0至不足1个百分点时可展示独立的“本次成本×100”参考，起点缺失时另适用下述明确标记的假设参考；达到完整步长后不能回退到该公式。新周期独立建立锚点，不混入旧周期成本。

界面已接受较新快照、后台才返回冻结校准时，只要账号、套餐、常规额度池和重置周期一致、百分比未倒退，仍显示这份有效冻结校准及其截至时间，等待新边界复核；不改写成本端点，也不允许较旧校准覆盖较新的既有结果。已观察到完整增量后，暂时缺快照也不会重新启用1%折算。

5小时与周额度分别显示实际校准的百分比区间，以及本地时间的统计起止时间（起点不含、终点包含）。沿用旧估算时仍显示旧证据的原始区间，不把当前百分比或主测量起点当成校准端点。1%参考值单独显示“参考成本统计区间”，对应主测量成本范围，不冒充严格校准区间；缺失端点显示“未提供”。

周卡还独立显示“本次用量区间16%→18% · 区间2%”。该百分比变化不依赖成本是否通过周期归属校验；成本不可靠时仍保留已验证的区间，但不伪造美元。空的结果起点字段可回退到同账号的冻结基线；同一重置时间的不同时区表示使用规范化周期标识匹配。无法验证起点时明确显示缺少有效快照，不把未知增量当成1%。

用户可接受的例外是**起点缺失时的假设参考**：有正的本次成本、同账号有效的当前常规周窗口，且尚未观察到完整增量时，单独展示“周总额度假设参考≈本次成本×100”，明确注明“假设1%折算、起点缺失、尚未校准、实际百分比增量未知”。例如本次成本约$0.1936，可显示约$19.36的假设参考；这不证明已消耗1%，也不证明套餐真实美元额度。内部实际增量仍为空，不推算已用/剩余美元，不改变严格校准。已有同周期有效估算优先；获得可靠起止快照后退出未知增量假设，完整增量一经确认便不能重新启用1%兜底。账号、计划、额度池、重置周期冲突或无效快照不适用此例外。
...
#### 订阅套餐折算美元参考（2026-09-26）

主卡、项目/对话查询和周期用量继续显示官方 **API 等价美元**，不把套餐 credits 倍率写成 API 单价。新启动的主测量窗口中，5 小时/周额度改用用户选择的 **套餐折算美元参考**，与 API 金额分开保存、标记和计算：

```text
每个调用的普通 API 估值 = 未缓存输入 × 普通输入价
                       + 缓存输入 × 普通缓存价
                       + 输出 × 普通输出价
套餐折算参考成本 = Σ(每个调用的普通 API 估值 × 对应套餐模式倍率)
...
例如普通 API 估值为 $10 的 GPT-6 Fast 调用，API 卡为 $20，套餐折算参考为 $25；若同边界额度增长 5 个百分点，套餐折算总额度参考为 $500。它不能证明套餐实际提供 $500 的现金额度。完整校准仍遵守固定锚点、父子规范事件去重、账号/套餐/额度池/重置周期边界；不足1%的单独参考也改用该套餐口径并明确标识。

升级不重建索引、不改写原日志。新口径由新启动的界面显式启用，已运行的旧窗口保持原 API 口径，避免运行途中混用。价格规则来源及核对日期独立保存在 `pricing.json` 的 `subscriptionPricing`，不会覆盖官方 API 价格。
