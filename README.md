...
**套餐规则不等于本地一定有窗口数据。** 5 小时卡仍只使用日志实际提供的、有效且未过期的 `codex` 池窗口；未提供时显示暂无，不按套餐制造百分比、重置时间或美元额度。企业计费细分、Pro 倍率等若没有可靠日志证据，仍需用户在官方用量页面确认。
...
人工选择只保存在本次程序运行的测量状态中，不写入日志、索引或周期历史；它覆盖本次测量及额度校准快照区间的对应模型，开始新测量后恢复全部普通模式。改变选择会清除旧计价假设下的额度结果并后台重算；结束后的重算仍使用原先冻结的结束边界。主动选择“未确认”时仍可显示带标注的普通价参考额度；没有真实额度窗口或可靠校准数据时不编造美元数值。
...
额度校准在冻结偏移内采用两阶段固定锚点。例如从26%开始，未到27%暂用历史25%→26%；到27%使用26%→27%首次校准，随后固定27%快照为累计起点：到28%使用27%→28%的成本除以1%，到30%使用27%→30%的累计成本除以3%。到达锚点但尚无后续增量时保持首次校准结果。锚点之后使用日志实际百分比增量（包括0.1%或0.01%），不再滑动取最近1%，也不补造缺失快照。同百分比重复快照不延长成本终点。仅聚合所选两端快照之间的成本，主测量Token和美元仍覆盖完整测量范围。没有可靠历史对、百分比下降或价格不完整时不生成新估算，仅保留同一有效窗口的旧结果；新周期独立建立锚点，不混入旧周期成本。
