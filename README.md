...
推理强度（包括 Ultra）不等于 Fast。日志未提供可靠的模式字段时，API 成本仍可显示普通价参考，但该参考成本不能直接生成新的美元额度估算。
...
人工选择只保存在本次程序运行的测量状态中，不写入日志、索引或周期历史；它覆盖本次测量及额度校准快照区间的对应模型，开始新测量后清空。改变选择会清除旧计价假设下的额度结果并后台重算；结束后的重算仍使用原先冻结的结束边界。若无法确认所有相关模型的模式，保留普通价参考，不假装已经得到准确的 Fast 美金额度。
