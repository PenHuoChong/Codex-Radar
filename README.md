...
额度校准始终在冻结偏移内选择同一计划、窗口和重置周期中最近完成的真实百分比步长，不再优先使用测量起点到当前的累计跨度。例如从26%开始且尚未到27%时可回查25%→26%；到27%改用26%→27%，到28%改用27%→28%。仅聚合所选两端快照之间的成本，主测量的Token和美元仍覆盖完整测量区间。只有整数百分比时通常使用最近的完整1%；日志实际出现0.1%或0.01%变化时直接使用该真实较细步长，不补造缺失百分比快照。没有可靠历史对、百分比下降、跨重置、计划变化或价格不完整时不生成新估算，并仅保留同一有效窗口的旧结果。
