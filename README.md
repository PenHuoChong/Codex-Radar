...
点击“查看结果”后，只要本次测量已有正的、有限的套餐折算美元参考成本，就可以显示周总额参考值（未启用新口径的旧调用仍使用 API 成本）：
...
- 周窗口缺失、冲突且尚未确认完整增量时，仍可显示上述成本参考值；它不证明套餐实际容量，不伪造百分比、重置时间或已用/剩余美元。价格仅部分可知时保留“部分”标记。
...
人工选择只保存在本次程序运行的测量状态中，不写入日志、索引或周期历史；它覆盖本次测量及额度校准快照区间的对应模型，开始新测量后恢复全部普通模式。改变选择会清除旧计价假设下的额度结果并后台重算；结束后的重算仍使用原先冻结的结束边界。主动选择“未确认”时仍可显示带标注的普通价参考额度；尚未观察到完整增量且周窗口或严格校准缺失时可使用明确标记的1%参考策略。已达到1个百分点后不再退回1%参考，不将它冒充日志证明的额度。
...
严格额度校准在冻结偏移内采用两阶段固定锚点。例如从26%开始，未到27%且已有成本时显示“本次成本×100”的1%参考值，取代此前优先显示历史25%→26%的方式；到27%使用26%→27%首次校准，随后固定27%快照为累计起点：到28%使用27%→28%的成本除以1%，到30%使用27%→30%的累计成本除以3%。到达锚点但尚无后续增量时保持首次校准结果。锚点之后使用日志实际百分比增量（包括0.1%或0.01%），不再滑动取最近1%，也不补造缺失快照。同百分比重复快照不延长严格成本终点。仅聚合所选两端快照之间的严格成本，主测量Token和美元仍覆盖完整测量范围。没有可靠历史对、百分比下降或价格不完整时不生成新的严格估算；仅在尚未确认完整增量时可展示独立1%参考。新周期独立建立锚点，不混入旧周期成本。
