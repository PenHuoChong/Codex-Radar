...
推理强度（包括 Ultra）不等于 Fast。日志未提供可靠的模式字段时，只要模型价格完整、额度快照边界有效且有实际百分比增量（或可回查历史完整步长），查询后直接显示美元额度，并标注“未知模式按普通价参考”。内部仍保持模式证据不完整，不将该结果当作已确认的 Fast 额度。
...
人工选择只保存在本次程序运行的测量状态中，不写入日志、索引或周期历史；它覆盖本次测量及额度校准快照区间的对应模型，开始新测量后清空。改变选择会清除旧计价假设下的额度结果并后台重算；结束后的重算仍使用原先冻结的结束边界。模式确认不是显示美元的前置条件；没有真实额度窗口或可靠校准数据时不编造美元数值。
