...
主界面“项目 / 对话用量…”打开独立窗口。点击项目查看项目汇总，展开后点击对话查看该对话及其子代理的消耗，沿用累计状态和父子任务去重、逐模型及长上下文计价。右侧同时显示总 Token、API 等价美元和各模型明细。
...
- 对话默认显示短会话 ID。点击“读取对话名称…”并确认后，只读本机 `session_index.jsonl` 的 ID 和 `thread_name` 标题元数据；没有标题仍使用 ID。标题仅保留在窗口内存中，不读取正文生成标题、不读密钥、不写入索引。
