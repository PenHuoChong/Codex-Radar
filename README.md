...
严格额度校准在冻结偏移内采用两阶段固定锚点。例如从26%开始，已验证未到27%且已有成本时显示“本次成本×100”的1%参考值，取代此前优先显示历史25%→26%的方式；到27%使用26%→27%首次校准，随后固定27%快照为累计起点：到28%使用27%→28%的成本除以1%，到30%使用27%→30%的累计成本除以3%。到达锚点但尚无后续增量时保持首次校准结果。锚点之后使用日志实际百分比增量（包括0.1%或0.01%），不再滑动取最近1%，也不补造缺失快照。同百分比重复快照不延长严格成本终点。仅聚合所选两端快照之间的严格成本，主测量Token和美元仍覆盖完整测量范围。没有可靠历史对、百分比下降或价格不完整时不生成新的严格估算；仅在已验证增量为0至不足1个百分点时可展示独立1%参考。新周期独立建立锚点，不混入旧周期成本。
