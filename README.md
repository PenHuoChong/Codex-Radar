...
同百分比重复快照不延长严格成本终点。分子和分母始终对应同一冻结快照区间，主测量Token和美元仍覆盖完整测量范围；因此当前周百分比、本次用量区间及实际校准区间可以不同。刷新时持续展示同周期有效美元，直到新证据覆盖；缺少可靠证据时不捏造金额。仅在已验证本次增量为0至不足1个百分点时可展示独立的“本次成本×100”参考，达到完整步长后不能回退到该公式。新周期独立建立锚点，不混入旧周期成本。
